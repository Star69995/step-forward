...
- רישום והתחברות עם Google, Email+Password, או שם משתמש+סיסמה בלבד (חשבון אנונימי ללא מייל, שאינו ניתן לשחזור אם נשכחת הסיסמה) — עם בחירת סוג משתמש בהרשמה (מקבל/ת שירות או נותן/ת שירות) ובחירת שם משתמש ייחודי; ניתן להמיר חשבון בין רגיל לאנונימי ובחזרה דרך עמוד הפרופיל
- שיתוף תוכניות (כולן או נבחרות) עם נותני שירות שמורים לפי כתובת מייל מדויקת או שם משתמש מדויק, עם קביעת הרשאת צפייה או צפייה+עריכה שניתן לבטל בכל רגע
...
- `npm run verify:rules` (`scripts/verify-firestore-rules.mjs`) מריץ סוללת בדיקות אוטומטית מול האמולטורים: הרשמה (כולל תפיסת שם משתמש ייחודי ב-`usernameIndex`), שיתוף לפי מייל או לפי שם משתמש, הרשאת צפייה/עריכה, הערות, וביטול שיתוף מיידי — כולל השאילתה שה-`ProviderSwitcher` תלוי בה. יש להריץ אותו אחרי כל שינוי ב-`firestore.rules`.
